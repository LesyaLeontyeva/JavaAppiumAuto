...
//        );
    ...
    private int ElementCount(By by, String error_message) {
        List<WebElement> articles = driver.findElements(by);
        return articles.size();
    }

    private void WordElement(By by, String value) {
        List<WebElement> articles = driver.findElements(by);
        for (WebElement temp : articles) {
            String txt = temp.getAttribute("text");
            boolean isContain = txt.contains(value);
            Assert.assertEquals(
                    "No needed word",
                    true,
                    isContain);
